...
public class Sim implements ChangeDayListener, TickListener {
    ...
    public Sim(SimSaver s){

        name = s.getName();
        currentHouse = new House(s.getCurrentHouse()); 
        currentLocation = s.getCurrentLocation();
        action = new Action(this);
        inventory = new InventoryManager(this, s.getInventory());
        status = new Status(s.getStatus());
        timeSinceLastSleep = s.getTimeSinceLastSleep();
        timeSinceLastSupper = s.getTimeSinceLastSupper();
